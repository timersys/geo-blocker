...
class Geobl_Rules {
	...
	public static function init() {
		...
		// Geotargeting
		add_filter( 'geobl/rules/rule_match/country', array( self::class, 'rule_match_country' ) );
		add_filter( 'geobl/rules/rule_match/country_region', array( self::class, 'rule_match_country_region' ) );
		...
		add_filter( 'geobl/rules/rule_match/logged_user', array( self::class, 'rule_match_logged_user') );
		add_filter( 'geobl/rules/rule_match/left_comment', array( self::class, 'rule_match_left_comment') );
		add_filter( 'geobl/rules/rule_match/search_engine', array( self::class, 'rule_match_search_engine') );
		add_filter( 'geobl/rules/rule_match/same_site', array( self::class, 'rule_match_same_site') );

		// Post
		add_filter( 'geobl/rules/rule_match/post_type', array( self::class, 'rule_match_post_type') );
		add_filter( 'geobl/rules/rule_match/post_id', array( self::class, 'rule_match_post') );
		add_filter( 'geobl/rules/rule_match/post', array( self::class, 'rule_match_post') );
		add_filter( 'geobl/rules/rule_match/post_category', array( self::class, 'rule_match_post_category') );
		add_filter( 'geobl/rules/rule_match/post_format', array( self::class, 'rule_match_post_format') );
		add_filter( 'geobl/rules/rule_match/post_status', array( self::class, 'rule_match_post_status') );
		add_filter( 'geobl/rules/rule_match/taxonomy', array( self::class, 'rule_match_taxonomy') );

		// Page
		add_filter( 'geobl/rules/rule_match/page', array( self::class, 'rule_match_post') );
		add_filter( 'geobl/rules/rule_match/page_type', array( self::class, 'rule_match_page_type') );
		add_filter( 'geobl/rules/rule_match/page_parent', array( self::class, 'rule_match_page_parent') );
		add_filter( 'geobl/rules/rule_match/page_template', array( self::class, 'rule_match_page_template') );

		//Other
		add_filter( 'geobl/rules/rule_match/custom_url', array( self::class, 'rule_match_custom_url') );
		add_filter( 'geobl/rules/rule_match/mobiles', array( self::class, 'rule_match_mobiles') );
		add_filter( 'geobl/rules/rule_match/tablets', array( self::class, 'rule_match_tablets') );
		add_filter( 'geobl/rules/rule_match/desktop', array( self::class, 'rule_match_desktop') );
		add_filter( 'geobl/rules/rule_match/referrer', array( self::class, 'rule_match_referrer') );
		add_filter( 'geobl/rules/rule_match/crawlers', array( self::class, 'rule_match_crawlers') );
		add_filter( 'geobl/rules/rule_match/query_string', array( self::class, 'rule_match_query_string') );
	}
	...
	*/
		...
		foreach ( $rules as $group_id => $group ) {
			...
			if ( is_array( $group ) ) {
				foreach ( $group as $rule_id => $rule ) {
					$match = apply_filters( 'geobl/rules/rule_match/' . $rule['param'], $rule );
					if ( ! $match ) {
						$match_group = false;
	...
	public static function set_rules_fields() {
		// GEO
		add_action( 'geobl/rules/print_country_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		add_action( 'geobl/rules/print_country_region_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		...
		add_action( 'geobl/rules/print_logged_user_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		add_action( 'geobl/rules/print_left_comment_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		add_action( 'geobl/rules/print_search_engine_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		add_action( 'geobl/rules/print_same_site_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );

		// Post
		add_action( 'geobl/rules/print_post_type_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		add_action( 'geobl/rules/print_post_id_field', array( 'Geobl_Helper', 'print_textfield' ), 10, 1 );
		add_action( 'geobl/rules/print_post_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		add_action( 'geobl/rules/print_post_category_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		add_action( 'geobl/rules/print_post_format_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		add_action( 'geobl/rules/print_post_status_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		add_action( 'geobl/rules/print_taxonomy_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );

		// Page
		add_action( 'geobl/rules/print_page_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		add_action( 'geobl/rules/print_page_type_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		add_action( 'geobl/rules/print_page_parent_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		add_action( 'geobl/rules/print_page_template_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );

		//Other
		add_action( 'geobl/rules/print_custom_url_field', array( 'Geobl_Helper', 'print_textfield' ), 10, 1 );
		add_action( 'geobl/rules/print_mobiles_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		add_action( 'geobl/rules/print_desktop_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		add_action( 'geobl/rules/print_tablets_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		add_action( 'geobl/rules/print_crawlers_field', array( 'Geobl_Helper', 'print_select' ), 10, 2 );
		add_action( 'geobl/rules/print_referrer_field', array( 'Geobl_Helper', 'print_textfield' ), 10, 1 );
		add_action( 'geobl/rules/print_query_string_field', array( 'Geobl_Helper', 'print_textfield' ), 10, 1 );
	}
	...
	public static function get_rules_choices() {
		$choices = array(
			__( "Geotargeting", 'geobl' ) => array(
				'country'        => __( 'Country', 'geobl' ),
				'country_region' => __( 'Country Region', 'geobl' ),
				'city'           => __( 'City', 'geobl' ),
			...
			__( "User", 'geobl' )         => array(
				...
				'logged_user'   => __( "User is logged", 'geobl' ),
				'left_comment'  => __( "User never left a comment", 'geobl' ) . ' *',
				'search_engine' => __( "User came via a search engine", 'geobl' ),
				'same_site'     => __( "User did not arrive via another page on your site", 'geobl' ),
			),
			__( "Post", 'geobl' )         => array(
				'post'          => __( "Post", 'geobl' ),
				'post_id'       => __( "Post ID", 'geobl' ),
				'post_type'     => __( "Post Type", 'geobl' ) ,
				'post_category' => __( "Post Category", 'geobl' ),
				'post_format'   => __( "Post Format", 'geobl' ) ,
				'post_status'   => __( "Post Status", 'geobl' ) ,
				'taxonomy'      => __( "Post Taxonomy", 'geobl' ),
			),
			__( "Page", 'geobl' )         => array(
				'page'          => __( "Page", 'geobl' ),
				'page_type'     => __( "Page Type", 'geobl' ),
				'page_parent'   => __( "Page Parent", 'geobl' ),
				'page_template' => __( "Page Template", 'geobl' ),
			),
			__( "Other", 'geobl' )        => array(
				'custom_url'   => __( "Custom Url", 'geobl' ),
				'referrer'     => __( "Referrer", 'geobl' ),
				'query_string' => __( "Query String", 'geobl' ),
				'mobiles'      => __( "Mobile Phone", 'geobl' ),
				'tablets'      => __( "Tablet", 'geobl' ),
				'desktop'      => __( "Dekstop", 'geobl' ),
				'crawlers'     => __( "Bots/Crawlers", 'geobl' ),
			)
		);

		// allow custom rules rules
		return apply_filters( 'geobl/metaboxes/rule_types', $choices );
	}
	...
	*/
	...
	public static function rule_match_search_engine( $rule ) {
		...
		$SE = apply_filters( 'geobl/rules/search_engines', array(
			'/search?',
			'.google.',
			'web.info.com',
			'search.',
